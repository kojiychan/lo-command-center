...
function safeEmailError(error: unknown, fallback = "Resend send failed.") {
  const message = error instanceof Error ? error.message : typeof error === "string" ? error : fallback;
  return message.replaceAll(resendApiKey() ?? "", "[redacted]");
}
...
export async function sendEmail(input: SendEmailInput) {
  ...
  const replyToEmail =
    ...
    normalizeEmail(settings?.reply_to_email) ||
  ...
  try {
    ...
    if (error) {
      const message = safeEmailError(error.message);
    ...
    const message = safeEmailError(error);
    await updateEmailStatus(initialLog?.id, { status: "failed", errorMessage: message });
    return { error: message };
  }
}
...
export async function sendSignupConfirmationEmail(input: {
  ...
  const fallback = DEFAULT_REMINDER_TEMPLATES.find((item) => item.template_key === "confirmation");
  const renderedSubject = renderEmailTemplate(
    template?.email_subject || fallback?.email_subject || `You're registered for ${webinar.title}`,
    context,
  );
  const renderedBody = renderEmailTemplate(
    template?.email_body || fallback?.email_body || `Hi {{first_name}},\n\nYou're registered for "{{title}}" at {{time}}.\n\nJoin link: {{join_link}}`,
    context,
  );
  const text = appendTextFooter(renderedBody, context);
  const html = buildHtmlEmail(renderedBody, context);
